...
run_bash_mac:
	docker run -p 5000:5000 --env SERVICE_ACCOUNT_KEY_BASE64="$(shell base64 < ./service_account.json)" --env PORT=5000 --env JWT_SECRET=testing1234 --env API_USERNAME=test --env API_PASSWORD=password123 -it gcloudrigapi bash
...
# macOS does --wrap=0 by default and does not have the command
run_mac:
	docker run -p 5000:5000 --env SERVICE_ACCOUNT_KEY_BASE64="$(shell base64 < ./service_account.json)" --env PORT=5000 --env JWT_SECRET=testing1234 --env API_USERNAME=test --env API_PASSWORD=password123 -it gcloudrigapi
